...
lap = cv.Laplacian(img, cv.CV_64F, ksize=1)  # CV_64F: 64-bit floating point number
lap = np.uint8(np.absolute(lap))

# Sobel Derivatives: 
sobelX = cv.Sobel(img, cv.CV_64F, 1, 0)  # 1: x-direction, 0: y-direction, 5th arguement can provide ksize
